...
- **引用来源 UI**：多路召回后即推送引用数据；卡片区（HTML）+ 引用列表表格（Dataframe）双展示，与 [1]-[5]/[G1] 一致；点击卡片或表格行均可跳转阅读 Tab 并定位 PDF 页码（表格通过 `ref_dataframe.select` → `jump_request_tb`）。
- **可点击引用**：回答内 [1][G1] 等与引用栏、引用面板卡片、引用表格行均支持一键跳转 PDF 对应页；Chunk 入库时正确写入 `page_number`（按节分块或默认 1），避免引用「全是 p.1」。
