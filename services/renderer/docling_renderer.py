...
class DoclingRenderer:
    ...
    def _render_metadata(self, metadata: Optional[Dict]) -> str:
        ...
        if "authors" in metadata:
            meta_items.append(
                f'<span class="meta-item">作者: {html.escape(str(metadata["authors"]))}</span>'
            )
        if "doi" in metadata:
            meta_items.append(
                f'<span class="meta-item">DOI: {html.escape(str(metadata["doi"]))}</span>'
            )
        if "page_count" in metadata:
            meta_items.append(
                f'<span class="meta-item">页数: {metadata["page_count"]}</span>'
            )
    ...
    def _apply_highlights(self, text: str) -> str:
        ...
        sorted_highlights = sorted(
            self.highlights, key=lambda h: len(h.get("content", "")), reverse=True
        )
    ...
    def _clean_table_html(self, html: str, index: int) -> str:
        ...
        # 添加样式类
        html = html.replace(
            "<table>", f'<table class="doc-table {self.options.table_style}">'
        )
